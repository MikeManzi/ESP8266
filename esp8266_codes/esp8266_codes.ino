...
const char* host = "localhost:5000"; //domain or subdomain of your website
...
void setup(){
  ...
  connectToWiFi("WIFI NAME", "WIFI PASSWORD");//Replace with your wifi name and password
...
void operateData(byte blockNumber, String initialBalance){
  ...
  */
  ...
  if(saveBalanceToCard(blockNumber, (unsigned char *)writingBuffer)==true){
    ...
    mData = "field1="+String(getUUID())+"&field2="+String(initialBalance)+"&field3="+String(transportFare);    

    connectToHost(80);

    transferData(mData, "/api/transaction");   
